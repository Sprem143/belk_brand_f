...
export default function S_return() {
   ...
   const [order, setOrder] = useState({
    ...
    const getprofile = async () => {
        ...
        try {
            ...
            if (admin.superadmin.mobile !== undefined) {
    ...
    const addproduct = async () => {
        try {
            let res = await fetch(`${localhost}/product/addreturnproduct`, {
                method: 'POST',
    ...
    const fetchproduct = async () => {
        let res = await fetch(`${localhost}/product/returnorder`, {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
        })
        res = await res.json()
...
{/* -----------edit form------------- */}
                        ...
                        <div className="d-flex mb-4  p-2 bg-primary text-white">
                            ...
                            </div>
                            {
                                ...
                                <div className="result">
                                    ...
                                    <th>Returned QTy</th>
                                    <th>V Cost</th>
                                    <th>Assigned to new AZ id</th>
                                    <th>Vendor Return Tracking ids</th> 
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {result.length > 0 && result.map((p, i) => (
                                                <tr className="products" key={i} style={{ minHeight: '350px !important' }}>
                                                    ...
                                                    <td>{p['Return date']}</td>
                                                    <td>{p['Original Row #']}</td>
                                        <td>{p['Condition']}</td>
                                        <td>{p['Return Received date']}</td>
                                        <td>{p['Return tracking id']}</td>
                                        <td>{p['AZ id']}</td>
                                        <td>{p['Old SKUs']}</td>
                                        <td>{p['Product']}</td>
                                        <td>{p['Return reason']}</td>
                                        <td>{p['Returned QTy']}</td>
                                        <td>{p['V Cost']}</td>
                                        <td>{p['Assigned to new AZ id']}</td>
                                        <td>{p['Vendor Return Tracking ids']}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </Table>
            ...
            <h1 className="text-end" onClick={downloadexcel}><Button>Download sheet</Button></h1>
            <Outlet />
        </div >
    )
}