...
export default function Products() {
    ...
    useEffect(() => {
        if (role === 'employee' || localStorage.getItem('gstar_employee')!==null) { getprofile(localStorage.getItem('gstar_employee')) }
        else if (role === 'admin' || localStorage.getItem('gstar_admin')!==null) { getadminprofile(localStorage.getItem('gstar_admin'))}
        else {
    ...
    const getadminprofile = async () => {
        try {
           let adminToken= localStorage.getItem('gstar_admin')
            ...
            if (employee.admin.mobile !== undefined) {
    ...
    return (
        <div className="ps-4 pe-4" style={{ opacity: loading ? 0.5 : 1, color: loading ? 'black' : null }}>
            ...
            <Button className="nobackgr ms-4" onClick={fetchproduct}><a href="#todayentry" style={{ textDecoration: 'none' }}>Today's Entry</a></Button>
