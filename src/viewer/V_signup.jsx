...
export default function V_signup() {
    ...
    useEffect(()=>{      
     ...
     else if(localStorage.getItem('gstar_admin')){
        getadminprofile()
     }
   ...
   try{
    ...
    const handleFormSubmit = async (e) => {
       if(name && mobile && password){
        ...
        try {
            let result = await fetch(`${localhost}/viewer/signup`, {
                method: 'POST',
            ...
            if (result.status) {
                alert('Viewer successfully signed up');
                navigate("/");
            }
            setLoading(false);
            return;
