...
export default function A_Notshipped() {
    ...
    const getprofile = async () => {
        try {
            ...
            let superadmin = await fetch(`${localhost}/admin/getprofile`, {
                method: 'GET',
                headers: { 'Authorization': `Bearer ${adminToken}` }
            });
            superadmin = await superadmin.json();
            console.log("no admin found")
            if (superadmin.admin.mobile !== undefined) {
                setProfile(superadmin.admin);
            } else {
    ...
    const fetchproduct = async () => {
        let res = await fetch(`${localhost}/product/notshipped`, {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
        })
        res = await res.json()
