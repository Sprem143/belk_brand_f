...
export default function A_return() {
    ...
    return (
        <div className="ps-4 pe-4" style={{ opacity: loading ? 0.5 : 1, color: loading ? 'black' : null }}>
            ...
            <Button className="ms-4 nobackgr"><Link to="/return" style={{ textDecoration: 'none' }} >All Entry</Link> </Button>
            <hr style={{ margin: 0 }} />
            {/* ---------show data---------- */}
            <Accordion className="mb-4 mt-4" defaultActiveKey={'0'}>
                <Accordion.Item eventKey="0">
                    <Accordion.Header>Total Number of Updated Products : &nbsp;&nbsp; <span style={{ color: 'blue' }}>{data.length > 1 ? data.length : 0}  <button className="nobackgr" onClick={downloadexcel}></button> </span></Accordion.Header>
                    <Accordion.Body>
